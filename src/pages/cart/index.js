...
const Cart = () => {
  const { cartItems, addToCart, removeItemFromCart, emptyCart } = useCart();
  const context = useContext(MyContext);

  const history = useNavigate();
  useEffect(() => {
    if (context.isLogin === "true") {
      // Cart data is already fetched using useCart hook
    } else {
      history("/");
    }
  ...
  const deleteItem = (id) => {
    // Remove item from the cart using useCart hook
    removeItemFromCart(id);
  };

  const updateCart = (items) => {
    // Update cart items using useCart hook
    // This might not be necessary based on your use case
    // because the useCart hook manages the cart state
  };
  ...
  const calculateTotal = () => {
    ...
    return cartItems.reduce(
      (total, item) =>
        total + parseInt(item.price.split(",").join("")) * item.quantity,
      0
    );
  };
